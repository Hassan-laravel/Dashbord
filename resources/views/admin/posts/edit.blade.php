                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control form-control-lg" name="{{ $currentLang }}[title]"
                                id="postTitle" onkeyup="handleTitleChange(this.value)"
                                value="{{ old($currentLang . '.title', $post->translate($currentLang)->title ?? '') }}"
                                required>
                        </div>
                ...
                <div class="card shadow-sm border-0 mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control bg-light" name="{{ $currentLang }}[slug]"
                                id="postSlug"
                                value="{{ old($currentLang . '.slug', $post->translate($currentLang)->slug ?? '') }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">{{ __('dashboard.categories.meta_title') }}</label>
                            <input type="text" class="form-control" name="{{ $currentLang }}[meta_title]"
                                id="postMetaTitle"
                                value="{{ old($currentLang . '.meta_title', $post->translate($currentLang)->meta_title ?? '') }}">
                        </div>
                        <div class="mb-3">
            ...
            <div class="col-lg-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="status">
                                <option value="published"
                                    ...
                                    {{ __('dashboard.posts.status_published') }}</option>
                                <option value="draft" {{ old('status', $post->status) == 'draft' ? 'selected' : '' }}>
                                    {{ __('dashboard.posts.status_draft') }}</option>
                            </select>
                        </div>
                ...
                <div class="card shadow-sm border-0 mb-3">
                    ...
                    <div class="card-body text-center">
                        @if ($post->image)
                            <img id="featured-preview" src="{{ Storage::disk('gcs')->url($post->image) }}"
                                class="img-fluid mb-2 rounded border"
                                style="max-height: 150px; width: 100%; object-fit: cover;">
                        @else
                            <img id="featured-preview" src="https://via.placeholder.com/300x200?text=No+Image"
                                class="img-fluid mb-2 rounded border">
                        @endif
                        <input type="file" class="form-control form-control-sm mt-2" name="image" accept="image/*"
                ...
                <div class="card shadow-sm border-0 mb-3">
                    ...
                    <div class="card-body">
                            ...
                            <div class="d-flex flex-wrap gap-2 mb-3">
                                @foreach ($post->images as $img)
                                    <div class="position-relative gallery-item-{{ $img->id }}">
                                        <img src="{{ Storage::disk('gcs')->url($img->image_path) }}" width="80"
                                            height="80" class="rounded border" style="object-fit: cover">

                                        <button type="button"
                                            class="btn btn-danger btn-sm position-absolute top-0 start-0 p-0 lh-1 rounded-circle"
                        ...
                        <input type="file" class="form-control form-control-sm" name="gallery[]" multiple
                            accept="image/*">
                    </div>
                </div>
            </div>
    ...
    <script>
        function deleteGalleryImage(imageId) {
            if (confirm("{{ __('dashboard.general.confirm_delete') }}")) {
                fetch(`/admin/posts/image/${imageId}`, {
                        method: 'DELETE',
                        headers: {
